...
function check() {
    ...
    /* Si no se ha encontrado la letra seleccionada, se desactiva el botón correspondiente y se actualiza el estado del juego. Si se han agotado los intentos, se indica que se ha perdido y se reincia.*/
    this.disabled = true;
    ...
    if (found === false) {
        ...
        // Si el total de fallos coincide con los fallos máximos lo advertimos y reiniciamos el juego
        if (numberOfTries == totalRights) {
            // Ambos eventos tienen un timeout para que de tiempo a terminar el muñeco y que salga alerta sin reinciar el juego
...
function disableButtons(buttons) {
    for (var index = 0, length = buttons.length; index < length; index++) {
        console.log("Entramos en desactivar botones");
